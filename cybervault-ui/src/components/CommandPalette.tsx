...
import { searchVault, SearchResult } from '@/lib/db/queries';
          ...
          <>
            ...
            <Command.Group heading={<div className="px-2 py-1 mt-2 text-xs font-bold text-gray-500 uppercase tracking-widest">Actions</div>}>
              <Command.Item onSelect={async () => { 
                setOpen(false); 
                const { createDailyNote } = await import('@/lib/db/queries');
              ...
              <Command.Item onSelect={async () => { 
                ...
                try {
                  ...
                  if (!settings?.htbAppToken) { alert('No API token configured. Go to Settings.'); return; }
